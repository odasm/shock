...
create table 'tsaip'.data_format_convert (
	...
	--oracle语句
alter table DATA_FORMAT_CONVERT add constraint primary  key (DATA_FORMAT_ID);
alter table DATA_FORMAT_CONVERT add constraint foreign key (SERV_DEFINE_ID) references SERV_DEFINE (SERV_DEFINE_ID);
...
create table 'tsaip'.serv_subscribe_conf (
	...
	--oracle语句
alter table SERV_SUBSCRIBE_CONF add constraint primary key (SERV_SUBSCRIBE_CONF_ID);
...
(
  ...
  ALARM_CANCEL_DETAIL VARCHAR(1000)
	...
	--oracle语句
alter table ALARM_TRANSMIT_HANDLE add constraint primary key (SERIAL_ID);
...
(
  ...
  SERV_DEFINE_ID  INT8
	...
	--oracle语句
alter table MSG_RESEND_RULE add constraint primary key (ID);
alter table MSG_RESEND_RULE add constraint foreign key (SERV_DEFINE_ID) references SERV_DEFINE (SERV_DEFINE_ID);
commit;
drop table serv_directory_level;
insert into serv_directory_level
  ...
  select serv_directory_level_id_seq.nextval,
          ...
          0,
          serv_man_id
     from serv_afford_man_config 
select * from serv_directory_level;
commit;